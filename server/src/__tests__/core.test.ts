...
function field(status: FieldResult['status']): FieldResult {
  return { fieldName: 'x', expectedValue: 'x', foundValue: 'x', status, notes: '' };
}

const validResult = {
  overallStatus: 'pass' as const,
  fields: [{ fieldName: 'brandName', expectedValue: 'Acme', foundValue: 'Acme', status: 'pass' as const, notes: '' }],
};
...
describe('deriveOverallStatus', () => {
  ...
  test('any warning → warning', () => {
    assert.equal(deriveOverallStatus([field('pass'), field('warning')]), 'warning');
  });
  ...
  test('fail beats warning', () => {
    assert.equal(deriveOverallStatus([field('warning'), field('fail')]), 'fail');
  });
});
...
describe('tryParse', () => {
  ...
  test('JSON wrapped in prose → extracted', () => {
    const result = tryParse(`Here is the analysis:\n${JSON.stringify(validResult)}\nHope that helps.`);
    assert.ok(result);
    assert.equal(result.overallStatus, 'pass');
  });
  ...
  test('empty fields array violates min(1) → null', () => {
    assert.equal(tryParse(JSON.stringify({ ...validResult, fields: [] })), null);
  });

  test('invalid overallStatus value → null', () => {
    assert.equal(tryParse(JSON.stringify({ ...validResult, overallStatus: 'approved' })), null);
  });
});
