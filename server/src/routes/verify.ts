...
router.post('/', verifyLimiter, withMulter, async (req: Request, res: Response, next: NextFunction) => {
  try {
    if (!req.file) {
    ...
    try {
      const result = await verifyLabel(imageBase64, mediaType, appResult.data, controller.signal);
      return res.json(result);
    } catch (err) {
      if ((err instanceof Error && err.name === 'AbortError') ||
          err instanceof Anthropic.APIConnectionTimeoutError) {
        return res.status(503).json({ error: 'Verification timed out, please try again' });
      }
      throw err;
    } finally {
      clearTimeout(timer);
