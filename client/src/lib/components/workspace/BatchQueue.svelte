            ...
            <div
                ...
                <div class="min-w-0">
                    ...
                    <p class="mt-0.5 text-xs font-medium text-gray-500">
                        Queue multiple labels for batch review and verification.
                    </p>
                </div>
                <!-- <div class="flex flex-wrap items-center gap-2">
                    <Button
            ...
            <div class="flex min-h-0 flex-1 overflow-hidden px-1 py-2">
                {#if files.length > 0}
                    <div
                        ...
                        role="list"
                        aria-label="Uploaded labels queued for batch review">
                        {#each files as file, index}
                            ...
                            <div role="listitem" class="shrink-0">
                                <button
                                    type="button"
                                    class="group flex min-w-[11rem] max-w-[11rem] flex-col overflow-hidden rounded-md border bg-white text-left shadow-sm transition {selected
                                        ? 'border-blue-500 ring-2 ring-blue-100'
                                        : 'border-gray-200 hover:border-blue-300 hover:bg-blue-50/30'} focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-blue-600"
                                    onclick={() => selectItem(index)}
                                    title={file.name}>
                                    <div
                                        class="h-24 w-full overflow-hidden border-b border-gray-100 bg-gray-50">
                                        {#if url}
                                            <img
                                                src={url}
                                                alt=""
                ...
                {:else}
                    <div
                        class="relative flex min-h-0 flex-1 items-center justify-center overflow-hidden rounded-md border-2 border-dashed border-gray-200 bg-white text-center">
                        <div
                            class="absolute left-4 top-1/2 hidden -translate-y-1/2 gap-2 [mask-image:linear-gradient(to_right,black_0%,black_58%,transparent_100%)] sm:flex"
                            aria-hidden="true">
                            {#each Array(5) as _}
                                <div
                                    class="h-24 w-24 rounded border border-dashed border-gray-200 bg-white shadow-sm">
                                </div>
                            {/each}
                        </div>
                        <div
                            class="absolute right-4 top-1/2 hidden -translate-y-1/2 gap-2 [mask-image:linear-gradient(to_right,transparent_0%,black_42%,black_100%)] lg:flex"
                            aria-hidden="true">
                            {#each Array(6) as _}
                                <div
                                    class="h-24 w-24 rounded border border-dashed border-gray-200 bg-white shadow-sm">
                        ...
                        <div
                            class="relative z-10 rounded-md bg-white/90 px-5 py-3 backdrop-blur-sm">
                            <div
                            ...
                            <p class="mt-1 text-xs font-medium text-gray-600">
                                Uploaded labels will appear here for batch
                                review.
                            </p>
                        </div>
                    </div>
