<script lang="ts">
    ...
    async function runSpeedTest() {
        ...
        try {
            ...
            speedTestResults = [
                ...speedTestResults,
                { label: 'Server reachable', ms: Date.now() - t0, status: 'ok' },
            ]
        } catch {
            speedTestResults = [
                ...speedTestResults,
        ...
        const blob = new Blob(
            [Uint8Array.from(atob(onePixelJpeg), (c) => c.charCodeAt(0))],
            { type: 'image/jpeg' },
        )
        form.append('image', blob, 'speedtest.jpg')
        form.append(
            'application',
            JSON.stringify({ beverageType: 'distilled_spirits' }),
        )
        ...
        <div class="mx-auto max-w-3xl px-6 py-8">
            {#if activeTab === 'readme'}
                <div class="prose-doc">
                    ...
                    <p>
                        ...
                        prototype for the TTB AI developer role.
                    </p>
                    ...
                    <ul>
                        ...
                        <li>
                            AI vision via pluggable provider — Claude API
                            (default), Azure AI Foundry (production path), or
                            mock (demo/restricted networks)
                        </li>
                    </ul>
                    ...
                    <p>
                        ...
                        validated against a hardcoded statutory constant in
                        <code>server/src/constants/warnings.ts</code>, not
                    ...
                    <p>
                        All AI calls are <strong>backend-only</strong>. The
                        ...
                        <table>
                            ...
                            <tbody>
                                ...
                                <tr>
                                    ...
                                    <td>
                                        Production / agency-approved egress
                                    </td>
                                ...
                                <tr>
                                    ...
                                    <td>
                                        Demos, training, restricted networks
                                    </td>
                    ...
                    <p>
                        If <code>AI_PROVIDER</code> is not set, the server
                        defaults to <code>anthropic_direct</code> when
                        <code>ANTHROPIC_API_KEY</code> is present and falls back
                        ...
                        whether results are real or simulated.
                    </p>
                    <p>
                        <strong>Government network deployment:</strong> Direct
                        access to <code>api.anthropic.com</code> is not
                        acceptable in most government network environments. The
                        intended production path is to set
                        <code>AI_PROVIDER=azure_foundry</code>, provision an
                        ...
                        approved endpoint is available, fill in the
                        <code>AZURE_FOUNDRY_*</code> variables and the app works
                    ...
                    <p>
                        ...
                        <code>AI_PROVIDER=mock</code>. The mock provider returns
                        deterministic simulated responses with no outbound
                        traffic — every field is labeled
                        <code>[Mock — AI provider not active]</code> and the UI
                        shows a persistent warning banner. This is safe for
                        demos and training in air-gapped or restricted networks,
                        but must never be used for real compliance decisions.
                    </p>
                        ...
                        <table>
                            ...
                            <tbody>
                                <tr>
                                    ...
                                    <td
                                        >Provider selection —
                                        <strong>optional</strong>, defaults to
                                        <code>anthropic_direct</code> or
                                        <code>mock</code></td>
                                    <td
                                ...
                                <tr>
                                    <td
                                        ><code
                                            >AZURE_FOUNDRY_API_VERSION</code
                                        ></td>
                                    <td
                                        >Optional, defaults to
                                        <code>2024-02-01</code></td>
                    ...
                    <p>
                        Outbound AI traffic depends on the active provider:
                    </p>
                    <ul>
                        <li>
                            <code>anthropic_direct</code>: outbound to
                            <code>api.anthropic.com:443</code>
                        </li>
                        <li>
                            <code>azure_foundry</code>: outbound to your Azure
                            Foundry endpoint only
                        </li>
                        <li>
                            <code>mock</code>: no outbound AI traffic
                        </li>
                    </ul>
                    <p>
                        Redis runs locally. No other external traffic leaves the
                        server. To check what provider is currently active and
                        whether it's reachable, call
                        <code>GET /api/ai/health</code>. The frontend header
                        badge reflects this status on every page load.
                    </p>
                </div>
            {/if}
            ...
            {#if activeTab === 'design'}
                <div class="prose-doc">
                    ...
                    <p>
                        ...
                        interface rather than calling the Anthropic SDK directly from
                        verification logic. Three implementations exist:
                        <code>AnthropicProvider</code> (current prototype path),
                        <code>AzureFoundryProvider</code> (production/agency path),
                        and <code>MockProvider</code> (offline/demo). The active
                        provider is selected at startup from the
                        <code>AI_PROVIDER</code> environment variable. No other
                    ...
                    <p>
                        The mock provider was added specifically to address
                    ...
                    <p>
                        ...
                        <code>server/src/constants/warnings.ts</code>, not user
                    ...
                    <p>
                        <code>updateBatchProgress</code> is implemented as a
                    ...
                    <p>
                        <code>setLabelResult</code> and
                        <code>updateBatchProgress</code> are wrapped in a Redis
                    ...
                    <p>
                        Base64 images arriving with a data URL prefix (<code
                            >data:image/jpeg;base64,…</code
                        >) are stripped before the Anthropic API call. The API
                        rejects prefixed strings with a 400 — this is a common
                        silent failure point when images originate from browser
                        FileReader or certain multer configurations.
                    </p>

                    <h2>Provider errors never reach the browser raw</h2>
                    <p>
                        The route layer catches <code>ProviderError</code> by
                        kind (<code>timeout</code>, <code>auth</code>,
                        <code>rate_limit</code>, <code>endpoint_blocked</code>,
                        <code>unavailable</code>) and converts each to a safe,
                    ...
                    <h2>
                        overallStatus derived from field results, not the AI
                    </h2>
                    <p>
                        <code>overallStatus</code> is computed deterministically
                        ...
                        occasional model misclassification.
                    </p>

                    <h2>Retry is a fresh call, not multi-turn</h2>
                    <p>
                        On Zod validation failure, the retry sends a fresh call
                    ...
                    <p>
                        Provider timeout errors are caught explicitly at the
                        route layer and returned as
                        <code>504 Gateway Timeout</code> rather than a generic
                        <code>500</code>. The frontend handles these distinctly,
                    ...
                    <p>
                        If <code>processBatch</code> throws at the job level,
                        the <code>.catch</code> handler in the upload route
                        updates <code>job.status</code> to <code>failed</code>
                        in Redis rather than just logging. Prevents jobs getting
                        stuck in <code>processing</code> state forever when the
                        processor crashes entirely.
                    </p>
                    ...
                    <p>
                        ...
                        clean images while failing fast on bad ones.
                    </p>
                </div>
            {/if}
            ...
            {#if activeTab === 'performance'}
                <div class="prose-doc">
                    ...
                    <p>
                        This test measures real round-trip latency from your
                    ...
                    <div class="speed-test-box">
                        ...
                        {#if speedTestDone && speedTestResults.every((r) => r.status === 'ok')}
                            <p class="speed-note ok">
                                All checks passed. The app is reachable and
                                responding within expected latency.
                            </p>
                        {:else if speedTestDone}
                            <p class="speed-note err">
                                One or more checks failed. Check that the
                    ...
                    <p>
                        The backend doesn't wait for the AI to finish before
                        ...
                        <code>FieldResult</code> objects over a Server-Sent
                        ...
                        seconds even though full analysis takes 4–7 seconds.
                    </p>
                        ...
                        <table>
                            ...
                            <tbody>
                                ...
                                <tr>
                                    ...
                                    <td>1–2s</td>
                                    <td>
                                        Fast AI call; skips full verify on
                                        unreadable images
                                    </td>
                                </tr>
                                <tr>
                                    ...
                                    <td>~2s</td>
                                    <td>
                                        Stream begins as soon as the AI starts
                                        responding
                                    </td>
                                </tr>
                                <tr>
                    ...
                    <p>
                        ...
                        in roughly 15–25 seconds total.
                    </p>
                    ...
                    <p>
                        ...
                        through to full analysis and produce more
                        <code>not_found</code> results with lower confidence.
                        There's no in-flight image enhancement — quality that
                        comes in is quality that gets analyzed.
                    </p>

                    <h2>What affects the 5-second SLA</h2>
                    <ul>
                        <li>
                            <strong>Zod validation failure.</strong> A retry
                            doubles latency. This is rare (&lt;5% of calls) but
                            when it happens it blows past 5 seconds.
                        </li>
                        <li>
                            <strong>Image size.</strong> The client resizes
                            before upload, capping at 1600px on the long edge.
                            Larger originals take longer to resize in the
                            browser; the upload itself stays small.
                        </li>
                        <li>
                            <strong>Field count.</strong> Labels with more
                            optional fields (sulfite declaration, appellation,
                            age statement, etc.) produce longer AI responses and
                            take slightly longer to stream.
                        </li>
                        <li>
                            <strong>Provider latency.</strong> The app has no
                ...
                <div class="prose-doc">
                    ...
                    <ul>
                        ...
                        <li>
                            ...
                            <code>processing</code> state older than 30 minutes
                    ...
                    <p>
                        The <code>azure_foundry</code> provider validates config
                    ...
                    <h2>Rate limiting</h2>
                    <p>
                        Current <code>express-rate-limit</code> at 20 req/min
                    ...
                    <ul>
                        <li>
                            Batch processor is hardcoded to 3 concurrent AI
                            calls. A production system would make this
                            configurable and dynamically tied to provider rate
                            limits.
                        </li>
                        <li>
                            The 15-second timeout gives the AI room to respond
                    ...
                    <ul>
                        <li>
                            ...
                            resolves to <code>complete</code> rather than
                            <code>failed</code> since all labels are settled.
                            Consumers need to inspect individual
                            <code>BatchLabelItem</code> statuses to determine
                            outcome.
                        </li>
                        <li>
                            Each SSE <code>label</code> event includes the full
                            <code>VerificationResult</code> inline. For large
                            batches this is verbose. A production implementation
                            would stream lightweight status updates and have the
                            client fetch full results on demand.
                        </li>
                    </ul>

                    <h2>Retry quality</h2>
                    <p>
                        For structural JSON failures, a multi-turn AI call
                    ...
                    <p>
                        <code>crypto.randomUUID()</code> requires Node 15+.
                        Ensure the deployment environment meets this requirement.
                    </p>
                </div>
            {/if}
        </div>
